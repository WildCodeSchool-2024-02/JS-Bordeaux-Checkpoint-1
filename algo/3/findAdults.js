...
function findAdults(persons) {
  const adultWoman = persons.filter(person => person.age >= 18 && person.sex === 'female');
  const adultMen = persons.filter(person => person.age >= 18 && person.sex === 'male');
  return [adultWoman, adultMen];
}
